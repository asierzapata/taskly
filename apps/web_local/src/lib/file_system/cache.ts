...
export const getLastWriteCacheData = async (
	...
	const jsonFile = await idbGet<string>(getLastWriteCacheKey(name))
	if (jsonFile) {
		const parsedJson = JSON.parse(jsonFile) as unknown
		if ((parsedJson as LastWriteCacheData).name) {
			return parsedJson as LastWriteCacheData
		}
